module.exports = {
	beforeCreate() {
		...
		let { vk, $store } = this;
		if(typeof $store !== "undefined" && !vk.$store){
			vk.$store = $store;
			vk.vuex = (name, value) => {
				vk.$store.commit('updateStore', { name, value });
			};
			/**
			 * (推荐) 设置vuex
			 ...
			 */
			vk.vuex.get = (name) => {
				let value = vk.pubfn.getData(vk.$store.state, name);
				if(typeof value === "undefined") return "";
				return JSON.parse(JSON.stringify(value));
			};
			vk.vuex.getters = (name) => {
				return vk.$store.getters[name];
			};
			/**
			 * 触发vuex的指定actions(异步)
